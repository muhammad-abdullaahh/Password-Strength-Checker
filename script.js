...
togglePassword.addEventListener('click', () => {
    ...
    // Toggle icon
    const iconPath = type === 'password' ? eyeOpenPath : eyeClosedPath;
    eyeIcon.querySelector('path').setAttribute('d', iconPath);
});
...
passwordInput.addEventListener('input', () => {
    ...
    for (const key in checks) {
        if (checks[key]) {
            hints[key].classList.add('met');
            strength++;
        } else {
            hints[key].classList.remove('met');
        }
    }
    ...
    if (password.length === 0) {
        ...
        // Shake if very weak after 4 chars
